...
//			android.os.SystemClock.sleep(5000*1);
	...
	public void timerTask() {
		...
		timer.schedule(new TimerTask() {
			public void run() {
				System.out.println("The value of shared preference is "
						+ getSharedPreferences("timertask", 0).getString("startservice", "").equals("timerservicestarted"));
...
//			 sms.sendTextMessage("7464", null, messageText, sentPI,deliveredPI);
			break;
		case 404:
			sendTxtMessage(mContext, slotId, new Intent(SENT), "+919212230707",messageText);
...
//			 sms.sendTextMessage("+919212230707", null, messageText, sentPI,deliveredPI);
	...
	public String tabletReg(String serial_no, String netinfo) {
		try {
			...
			if (Setting.IS_LOLLIPOP) {
				...
				// TelephonyInfo.getBuild(TimerTaskService.this,"android.os.SystemProperties", "ro.custom.build.version");
		...
		String regMessage = "REG:01:01" + netarray[0] + ":02" + netarray[1]+ ":03" + netarray[2] + ":04" +  "TGP469GRY" + ":05"+ serial_no + ":06" + "HW_V1.6" + ":07" + softwareVersion + ":";

		String checkSum = checkSumGenerator(regMessage);
		regMessageChecksum = "REG:01:01" + netarray[0] + ":02" + netarray[1]+ ":03" + netarray[2] + ":04" + "TGP469GRY" + ":05"+ serial_no + ":06" + "HW_V1.6" + ":07" + softwareVersion + ":"+ checkSum;
	...
	public String getSignalInfo(int simId) {
		...
		if(simId==1 && model.contains("A99")){
			...
			GsmCellLocation loc = (GsmCellLocation) telephonyInfo.getCellLocation();
			if (loc != null) {
				cid = loc.getCid();
				lac = loc.getLac();
...
//					getContentResolver().delete(Uri.parse("content://sms/sent"),"body = ?", new String[] { tabletRegs });
					if( !getSharedPreferences("locValues", 0).getString("Serverloc","").equals("message sent wifi")){
						Intent i = new Intent("com.package.MY_DIALOG");
						i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
						startActivity(i);
